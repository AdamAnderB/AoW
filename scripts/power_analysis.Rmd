...
dat <- expand.grid(n_participants = c(40,50,60), n_trials = c(15,25,35))
dat$id <- 1:nrow(dat)

true_slope_value <- 0.1  # Example true slope, adjust this based on your needs
true_effect_size <- 0.28074  # Example effect size based on previous estimates

results_1 <- dat %>%
    mutate(
        simulations = pmap(list(n_participants, n_trials),
                           ~ repeat_analysis(n_simulations = 2, alpha = 0.05, theta = true_effect_size,
                                             glmm_model = glmm5_1,
