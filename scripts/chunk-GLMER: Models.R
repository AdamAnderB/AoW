## ----GLMER: Models----
m1_1<-glmer(target~talker*verb_type*experience_chinese+
...
#remove correlations
m1_2<-glmer(target~talker*verb_type*experience_chinese+
            (talker||subject_img_file)+
            (verb_type||Participant.Private.ID)+
            (1|time_normalized),
          family="binomial",data=mem_data)
#remove timing random effect
m1_3<-glmer(target~talker*verb_type*experience_chinese+
            (talker||subject_img_file)+
            (verb_type||Participant.Private.ID),
          family="binomial",data=mem_data)
